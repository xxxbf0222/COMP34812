...
class SimpleLstmClassifier(nn.Module):
    ...
    def _encode(self, embedded_seq, lengths):
        packed_seq = pack_padded_sequence(embedded_seq, lengths.cpu(), batch_first=True, enforce_sorted=False) # sequence are packed here
...
class LstmAttentionClassifier(nn.Module):
    ...
    def forward(self, claim_ids, claim_lens, evidence_ids, evidence_lens):
        """
        claim_ids, evidence_ids: [B, Lc] / [B, Le]
        claim_lens, evidence_lens: [B]
        
        return: logits, probabilities: [B, num_classes]
        """
        # ========== 1. Embedding Layer ==========
